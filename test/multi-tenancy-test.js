...
describe('Contents in different tanant', () => {
    it('it should GET all the tags of a specific tenant', async () => {
        ...
        const tag1_tenant_1 = await factory.createTag("test-tag-1", user_tenant_1, UsageTypes.default, tenant_1);
        const tag2_tenant_1 = await factory.createTag("test-tag-2", user_tenant_1, UsageTypes.default,tenant_1);
        const tag3_tenant_1 = await factory.createTag("test-tag-3", user_tenant_1, UsageTypes.default,tenant_1);
        const tag1_tenant_2 = await factory.createTag("test-tag-1", user_tenant_2, UsageTypes.default,tenant_2);
        const tag2_tenant_2 = await factory.createTag("test-taf-2", user_tenant_2, UsageTypes.default,tenant_2);
